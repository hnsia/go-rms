...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if invoice.Payment_method != nil {
			// err := menuCollection.FindOne(ctx, bson.M{"payment_method": invoice.Payment_method}).Decode(&table)
			// defer cancel()
			// if err != nil {
		...
		if invoice.Payment_status != nil {
			// err := menuCollection.FindOne(ctx, bson.M{"payment_status": invoice.Payment_status}).Decode(&table)
			// defer cancel()
			// if err != nil {
			// 	msg := fmt.Sprintf("Menu was not found")
			// 	c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
